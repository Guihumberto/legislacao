        ...
        <div class="container2">
          <v-container>
            ...
            <v-row>
              ...
              <!-- <ActionsPrompt :prompt="prompt" v-if="prompt" :dialog2="dialog" /> -->
                    ...
                    <div v-else-if="viewMode === 'normas'">
                      <v-list lines="two">
                        <template v-for="(disciplina, disciplinaIndex) in disciplinas" :key="disciplinaIndex">
                          <v-list-subheader class="text-h6 text-black"> <v-icon>mdi-arrow-right</v-icon> {{ disciplina.nome }}</v-list-subheader>
...
`,
  ...
  computed:{
    textoInit(){
      const texto = `Você é um professor experiente em concursos publicos. Quais os assuntos mais relevantes para se preparar para o  cargo de ${this.cargo} do concurso de ${this.concurso}, que vai ser realizado pela  ${this.banca} no ano ${this.ano},
      a disciplina da analise e seus assuntos estao relacionados abaixo. Liste os assuntos que devo focar(se interessante use pareto com método), com base na recorrencia das questoes que cairam nos concursos anteriores da mesma banca e
      em cargos relacionados. Se possivel liste por cada assunto as sumulas e julgados importantes para os e faça um pequeno resumo. segue o resumo: `
      return texto
    }
  },
  methods: {
    ...
    selectItem(local, disciplina = null, topico = null, subtopico = null, subsubtopico = null) {
      // console.log('Item selecionado:', local, diciplina, topico, subtopico, subsubtopico);
      this.prompt = null
      if(local == 'disciplina') {
        const texto = this.createPrompt(disciplina.topicos, disciplina.nome)
