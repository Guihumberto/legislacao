<template>
    ...
    <div class="w-100 border rounded-lg">
        ...
        <v-form @submit.prevent="submitForm" ref="formref" style="margin: 2rem;">
            ...
            <v-textarea
                label="Array de Súmulas"
                density="compact"
                variant="outlined"
                v-model="form.listaSumulas"
                :rules="[rules.required]"
            ></v-textarea>
            ...
            <div v-for="(item, index) in listImport" :key="index" class="pa-2 mb-2 border rounded-lg">
                ...
                <div class="font-weight-bold">Tese:</div>
                {{ item.tese }} <br><br>
                <div class="font-weight-bold">Texto:</div>
                {{ item.texto }} <br>
            </div>
        </div>
...
<script setup>
    ...
    const form = ref({
        title: `Súmulas: ${props.selected?.numero} - ${props.selected?.conteudo}` || '',
        listaSumulas: '',
        nivel: 'medio'
    })
    ...
    const typeGuide = 'sumulas'
    const icon = 'mdi-account-school'
    ...
    const submitForm = async () => {
        ...
        const resp = isValidJsonString(form.value.listaSumulas)
