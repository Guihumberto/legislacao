<template>
    ...
    <div class="w-100 border rounded-lg">
        ...
        <v-form @submit.prevent="submitForm" ref="formref" style="margin: 2rem;">
            ...
            <v-textarea
                label="Array de Artigos"
                density="compact"
                variant="outlined"
                v-model="form.listArtigos"
                :rules="[rules.required]"
            ></v-textarea>
...
<script setup>
    ...
    const form = ref({
        title: `Artigos: ${props.selected?.numero} - ${props.selected?.conteudo}` || '',
        listArtigos: '',
        nivel: 'medio'
    })
    ...
    const typeGuide = 'artigos'
    const icon = 'mdi-numeric-5-box-multiple-outline'
    ...
    const submitForm = async () => {
        ...
        const resp = isValidJsonString(form.value.listArtigos)
