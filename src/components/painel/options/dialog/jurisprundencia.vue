            ...
            <v-card color="blue-grey-lighten-5">
                ...
                <v-card-text class="overflow-auto" style="max-height: 800px;">
                    <div class="jurisprudence-container" v-if="toggle == 0">
                        ...
                        <v-card 
                            v-for="(item, i) in jurisprudencias" 
                            :key="i" 
                    ...
                    <CardJuris
                        ...
                        :favGravados="jurisFavoritasGravadas"
                        :sofavs="soFavs"
...
<script setup>
    ...
    const jurisprudencias = computed(() => {
        ...
        if(soFavs.value) {
            jurisprudencias = jurisprudencias.filter((item, index) => jurisFavoritasGravadas.value.includes(index));
        }
    ...
    const onToggleFavorite = async (data) => {
        await optionsStore.favJuris({...props.conteudo, favoriteJuris: data.favorited})
    }
    ...
    const jurisFavoritasGravadas = computed(() => {
        const select = optionsStore.readJurisControle.find(item => item.guia_id === props.conteudo.id)
        return select ? select.favoriteJuris : []
    })

    const isFavorited = (index) => {
     return jurisFavoritasGravadas.value.includes(index)
    }
